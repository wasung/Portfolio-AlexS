...
source('~/Portfolio-AlexS/ReproducibleArticleFiles/format-data.r')
...
make_table <- function(filename){
  ...
  df_model['text'] <- paste0(df_model[,1]," [",df_model[,2]," - ",df_model[,3],"]")
  df_model <- cbind(df_model, countries)
  write.csv(df_model[,c('countries', 'text')], file = paste0('~/Portfolio-AlexS/ReproducibleArticleFiles/Results/per-cases-model-',filename, '.csv'))
}