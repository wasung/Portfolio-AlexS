...
if (requireNamespace("gmapR", quietly = TRUE)) {
  ...
  # Post-filter the variants that are too closely clustered
  pf.variants <- postFilterVariants(called.variants)

  # Subset variants to those in actual p53 exons
  calledbyoverlap.variants <- subsetByOverlaps(called.variants, p53, ignore.strand = TRUE)
}
